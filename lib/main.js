...
function run() {
    return __awaiter(this, void 0, void 0, function* () {
        let owner = core.getInput('owner', { required: false }) || github_1.context.repo.owner;
        let repo = core.getInput('repo', { required: false }) || github_1.context.repo.repo;
        ...
        try {
            let pr = yield octokit.pulls.create({ owner: github_1.context.repo.owner, repo: github_1.context.repo.repo, title: prTitle, head: owner + ':' + head, head_repo: repo, base: base, body: prMessage, maintainer_can_modify: false });
            yield delay(20);
            if (autoApprove) {
                yield octokit.pulls.createReview({ owner: github_1.context.repo.owner, repo: github_1.context.repo.repo, pull_number: pr.data.number, event: "COMMENT", body: "Auto approved" });
                yield octokit.pulls.createReview({ owner: github_1.context.repo.owner, repo: github_1.context.repo.repo, pull_number: pr.data.number, event: "APPROVE" });
            }
            if (autoMerge) {
                yield octokit.pulls.merge({ owner: github_1.context.repo.owner, repo: github_1.context.repo.repo, pull_number: pr.data.number, merge_method: mergeMethod });
            }
        }
        catch (error) {
            ...
            if (!!error.errors && !!error.errors[0] && !!error.errors[0].message && error.errors[0].message.startsWith('No commits between')) {
                console.log('No commits between ' + github_1.context.repo.owner + ':' + base + ' and ' + owner + ':' + head);
            }
